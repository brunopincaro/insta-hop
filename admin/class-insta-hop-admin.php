...
class Insta_Hop_Admin {
	...
	public function __construct( $plugin_name, $version ) {
        ...
        $this->insta_hop_options = get_option($this->plugin_name);
	}
	...
	public function enqueue_styles() {
		 ...
		 * defined in Insta_Hop_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Insta_Hop_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/insta-hop-admin.css', array(), $this->version, 'all' );

        // ----------------- INSTAHOP
        if ( 'settings_page_insta-hop' == get_current_screen()->id ) {
             // CSS stylesheet for Color Picker
             wp_enqueue_style( 'wp-color-picker' );            
             wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/insta-hop-admin.css', array( 'wp-color-picker' ), $this->version, 'all' );
         }
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/insta-hop-admin.js', array( 'jquery' ), $this->version, false );

        // ------------------ INSTAHOP
        if ( 'settings_page_insta-hop' == get_current_screen()->id ) {
            wp_enqueue_media();   
            wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/insta-hop-admin.js', array( 'jquery', 'wp-color-picker' ), $this->version, false );         
        }
	}
    ...
    */
    ...
    public function add_plugin_admin_menu() {
        ...
        */
        ...
        //$capability : https://codex.wordpress.org/Roles_and_Capabilities#Capabilities
        add_options_page( 'InstaHOP Settings', 'InstaHOP', 'manage_options', $this->plugin_name, array($this, 'display_plugin_setup_page')
        );
    }
    ...
    */
    ...
    public function display_plugin_setup_page() {
        include_once( 'partials/insta-hop-admin-display.php' );
    }
    ...
    **/
     ...
     * @since    1.0.0
     */
     private function insta_hop_login_logo_css(){
         if(isset($this->insta_hop_options['login_logo_id']) && !empty($this->insta_hop_options['login_logo_id'])){
             $login_logo = wp_get_attachment_image_src($this->insta_hop_options['login_logo_id'], 'thumbnail');
     ...
     private function insta_hop_login_background_color(){
         if(isset($this->insta_hop_options['login_background_color']) && !empty($this->insta_hop_options['login_background_color']) ){
             $background_color_css  = "body.login{ background:".$this->insta_hop_options['login_background_color']."!important;}";
             return $background_color_css;
         }
     }
     // Get Button and links color is set and different from #00A0D2 return it's css
     private function insta_hop_login_button_color(){
         if(isset($this->insta_hop_options['login_button_primary_color']) && !empty($this->insta_hop_options['login_button_primary_color']) ){
             $button_color = $this->insta_hop_options['login_button_primary_color'];
     ...
     public function insta_hop_login_css(){
         if( !empty($this->insta_hop_options['login_logo_id']) || $this->insta_hop_login_background_color() != null || $this->insta_hop_login_button_color() != null){
             echo '<style>';
             if( !empty($this->insta_hop_options['login_logo_id'])){
                   echo $this->insta_hop_login_logo_css();
             }
             if($this->insta_hop_login_background_color() != null){
                   echo $this->insta_hop_login_background_color();
             }
             if($this->insta_hop_login_button_color() != null){
                   echo $this->insta_hop_login_button_color();
             }
             echo '</style>';
         }
     }
